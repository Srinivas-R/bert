  ...
  for (example_index, example) in enumerate(examples):
    ...
    for (doc_span_index, doc_span) in enumerate(doc_spans):
      ...
      if example_index < 10:
        ...
        tf.logging.info("doc tokens: %s" % " ".join(
            [tokenization.printable_text(x) for x in doc_tokens]))
...
def create_model(bert_config, is_training, doc_input_ids, 
                 query_input_ids, doc_input_mask, query_input_mask, 
                 doc_segment_ids, query_segment_ids, use_one_hot_embeddings):
  """Creates a classification model."""

  model_doc = modeling.BertModel(
      config=bert_config,
  ...
  doc_final_hidden = model_doc.get_sequence_output()

  doc_hidden_shape = modeling.get_shape_list(doc_final_hidden, expected_rank=3)
  batch_size = doc_hidden_shape[0]
  ...
  #tile it to concatenate it with each doc token repr
...
def input_fn_builder(input_file, seq_length, is_training, drop_remainder):
  ...
  name_to_features = {
      ...
      "query_input_ids": tf.FixedLenFeature([seq_length], tf.int64),
      "query_input_mask": tf.FixedLenFeature([seq_length], tf.int64),
      "query_segment_ids": tf.FixedLenFeature([seq_length], tf.int64)
  }
